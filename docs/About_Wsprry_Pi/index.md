...
| --- | --- | --- | --- |
| 2200 m | <span class="qualification-status qualification-status--partial">Partial</span><sup>1</sup> | <span class="qualification-status qualification-status--qualified">Qualified</span><sup>1</sup> | <span class="qualification-status qualification-status--qualified">Qualified</span><sup>2</sup> |
...
| 6 m | <span class="qualification-status qualification-status--unqualified">Unqualified</span> | <span class="qualification-status qualification-status--partial">Partial</span><sup>8</sup> | <span class="qualification-status qualification-status--qualified">Qualified</span> |
...
| 1.25 m | <span class="qualification-status qualification-status--unqualified">Unqualified</span> | <span class="qualification-status qualification-status--unavailable">Unavailable</span><sup>9</sup> | <span class="qualification-status qualification-status--unavailable">Unavailable</span><sup>6</sup> |
| 70 cm | <span class="qualification-status qualification-status--unqualified">Unqualified</span> | <span class="qualification-status qualification-status--unavailable">Unavailable</span><sup>9</sup> | <span class="qualification-status qualification-status--unavailable">Unavailable</span><sup>6</sup> |

1. On 2200 m, the legacy 500 MHz PLLD profile is qualified for TONE. QRSS, FSKCW, and DFCW are untested and remain blocked unless the explicit experimental override is active. Its WSPR evidence does not meet the requirement for three consecutive correct decodes: an earlier run reached one consecutive decode, and a follow-up three-frame run produced no correct decodes after a passing carrier control. The profile is therefore Partial rather than fully Qualified. The BCM2711 profile is qualified for all four CW-family modes and WSPR; it uses the 54 MHz oscillator because the 750 MHz PLLD cannot represent the required divider.
2. The Si5351 backend is qualified on 2200 m using CLK0 at minimum 2 mA drive with a 27 MHz external TCXO. The qualification was bracketed by passing 160 m carrier controls. The 137,500 Hz carrier was measured within +0.381 Hz of the request, and three consecutive WSPR frames decoded correctly. QRSS, FSKCW, and DFCW each passed three independently captured `TEST` repetitions; FSKCW retained continuous RF with 5.001 Hz tone separation, and DFCW used distinct dot and dash tones separated by approximately 5.08 Hz. Every credited capture completed without receiver overflow, and CLK0 was disabled after each run. The result applies to the tested WsprryPi signal-generation path; an appropriate band-specific low-pass filter remains required.
...
8. On 6 m, the BCM2711 750 MHz PLLD profile is qualified for TONE, QRSS, FSKCW, and DFCW. QRSS passed three independently captured keyed repetitions. FSKCW passed three shifted-tone repetitions with median separations of 4.574, 5.047, and 5.007 Hz. After recalibration to 13.0013 PPM, DFCW passed three shifted-tone repetitions with median separations of 5.158, 4.813, and 5.788 Hz, bracketed by passing carrier controls. Two separately bracketed three-frame WSPR attempts each produced only two consecutive correct decodes, short of the three required, so WSPR remains unqualified. The profile is therefore Partial.
9. On 1.25 m and 70 cm, the BCM2711 GPIO planner cannot construct the requested output. These combinations are unavailable and cannot be enabled by an experimental override.
